...
const api=crearApi(async()=>{if(cache.token&&cache.hasta>Date.now())return cache.token;cache={token:await stravaAccessToken(),hasta:Date.now()+50*60e3};return cache.token;});
...
export default async function handler(req,res){
 ...
 if(!process.env.STRAVA_REFRESH_TOKEN)return res.status(500).json({error:'Falta STRAVA_REFRESH_TOKEN en Vercel'});
 let body=req.body;
