// Herramientas de Strava compartidas por el MCP local (mcp/strava-mcp.js, por stdio) y el remoto (api/mcp/[key].js, por HTTP).
const API='https://www.strava.com/api/v3';

// Devuelve una función api(ruta, query) que llama a Strava con el token que dé getToken()
export function crearApi(getToken){
 return async function api(p,q){
  const u=new URL(API+p);Object.entries(q||{}).forEach(([k,v])=>{if(v!=null)u.searchParams.set(k,v)});
  const r=await fetch(u,{headers:{Authorization:'Bearer '+await getToken()}});
...
async function actividades(api,dias,tipo){
...
export const TOOLS=[
 {name:'strava_atleta',description:'Perfil del atleta (peso, zapatillas) y totales de carrera de las últimas 4 semanas, del año y de siempre.',inputSchema:{type:'object',properties:{}},annotations:RO},
 {name:'strava_actividades',description:'Lista de actividades recientes con km, ritmo, pulso medio/máximo, cadencia, desnivel y esfuerzo relativo. Por defecto carreras (Run) de los últimos 30 días.',inputSchema:{type:'object',properties:{dias:{type:'integer',description:'Días hacia atrás (por defecto 30, máximo 365)'},tipo:{type:'string',description:'Run (por defecto), Ride, WeightTraining, Walk… o all para todo'}}},annotations:RO},
 {name:'strava_actividad',description:'Detalle completo de una actividad: descripción, parciales por km (ritmo y pulso de cada km), vueltas, mejores esfuerzos (1k, 5k…), calorías, dispositivo y zapatillas.',inputSchema:{type:'object',properties:{id:{type:'integer',description:'id de la actividad (de strava_actividades)'}},required:['id']},annotations:RO},
 {name:'strava_zonas',description:'Tiempo en cada zona de pulso y de ritmo durante una actividad.',inputSchema:{type:'object',properties:{id:{type:'integer'}},required:['id']},annotations:RO},
 {name:'strava_streams',description:'Series temporales de una actividad (pulso, cadencia, ritmo, altitud, distancia) remuestreadas a N puntos para ver cómo evolucionó dentro de la sesión.',inputSchema:{type:'object',properties:{id:{type:'integer'},puntos:{type:'integer',description:'Número de puntos (por defecto 40, máximo 200)'}},required:['id']},annotations:RO},
 {name:'strava_semanas',description:'Resumen por semana (lunes a domingo) de las carreras: km, minutos, salidas, ritmo y pulso medios, tirada más larga, sesiones de series. Para ver carga y progresión.',inputSchema:{type:'object',properties:{semanas:{type:'integer',description:'Semanas hacia atrás (por defecto 8, máximo 52)'}}},annotations:RO}
];
export async function llamar(name,a,api){
 ...
 if(name==='strava_actividades'){const l=await actividades(api,Math.min(Number(a.dias)||30,365),a.tipo||'Run');return{total:l.length,actividades:l.map(resumen)};}
 ...
 if(name==='strava_semanas'){
  const w=Math.min(Number(a.semanas)||8,52);const l=await actividades(api,w*7+7,'Run');const g={};
...
// ---------- protocolo MCP ----------
// Atiende un mensaje JSON-RPC y devuelve la respuesta, o null si era una notificación (sin id).
const VERS=['2024-11-05','2025-03-26','2025-06-18'];
export async function atender(m,api){
 const {id,method,params}=m||{};
 if(id==null||!method)return null;
 if(method==='initialize')return{jsonrpc:'2.0',id,result:{protocolVersion:VERS.includes(params?.protocolVersion)?params.protocolVersion:VERS[VERS.length-1],capabilities:{tools:{}},serverInfo:{name:'strava-legua',version:'1.1.0'}}};
 if(method==='tools/list')return{jsonrpc:'2.0',id,result:{tools:TOOLS}};
 if(method==='tools/call'){
  try{const out=await llamar(params?.name,params?.arguments||{},api);return{jsonrpc:'2.0',id,result:{content:[{type:'text',text:JSON.stringify(out,null,1)}]}}}
